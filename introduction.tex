...
theory would be conceptually clarified over the following months by
...
\begin{equation}
    ...
    \Psi
\end{equation}
(Note that we are using atomic units.)
...
\end{equation}
The vector space containing the eigenfunctions of a given Hamiltonian is its
Hilbert space, \(\mathcal{H}\), which in this case is the space of square
integrable functions of \(n\) position variables,
\(
    L^2(\mathbb{R}^{3n})
\),
one for each electron in the molecule.
Since this space is infinite-dimensional, the only way to make progress in most
cases is to determine a basis that approximately spans the states of interest.
...
\begin{enumerate}
    ...
    \item
        ...
        Treating the other electrons in a molecule as fields which partially
        shield the nuclear charges, the states of individual electrons in a
    ...
    \item
        ...
        highest charge, with very little contribution from, say, the 42g orbital
        of another atom.
    ...
    \item
        ...
        From a statistical perspective, this means that the densities of
        electron probability density approximately separates into a product of
        one-electron distributions, because the molecular orbitals are {\itshape
        weakly correlated}.
\end{enumerate}
