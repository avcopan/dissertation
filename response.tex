...
\begin{table*}[h!]
    \caption{%
        ...
        Errors in vertical excitation energies (eV) for triplet states computed
        using LR-OLCCD, LR-ODC-12, and EOM-CCSD, relative to EOM-CCSDT
        (aug-cc-pVTZ basis set).
        All electrons were correlated in all computations.
        Also shown are mean absolute errors (\mae) and standard deviations
    ...
    \begin{threeparttable}
        ...
        \item[a]
            ...
            EOM-CC(2,3), which energies were shifted to reproduce the EOM-CCSDT
            energy for the ${}^1\Sigma^-$ (${}^1\Sigma_u^-$) state.
        \end{tablenotes}    
    \end{threeparttable}
\end{table*}
...
\begin{figure}
    \centering
    \caption{Mean absolute errors. {\color{red}(provide more description)}}
...
\begin{subappendices}
    \section{Differentiating the One-Body Density in Density Cumulant Theory}
    \label{sec:appendix}

    Repeated differentiation of the one-body \(n\)-representability condition in
    DCT (\cref{eq:one-body-n-rep}) gives the following formulas for first and
    second derivatives of the cumulant trace.
    \begin{equation}
        \begin{array}{r@{\,}l}
    ...
    \begin{equation}
        ...
        \end{array}
    \end{equation}
    In the natural spin-orbital basis (NSO, denoted by prime indices), where the
    one-body density matrix is diagonal, we can invert these equations to
    determine the first and second derivatives derivatives of the one-body
    density from the cumulant derivatives.
    \begin{equation}
    ...
    Substituting \cref{eq:one-body-n-rep-gradient,eq:one-body-n-rep-hessian}
    into the two-body energy Hessian in \cref{eq:odc12-hessian-initial-form} and
    simplifying  allows us to determine the intermediates in
    \cref{eq:a22_odc12}.
    In the NSO basis they are given by
    \begin{equation}
    ...
    \end{equation}
    but note that these quantities must be back-transformed to the variatonal
    orbital basis to be compatible with the other quantities in
    ...
    \begin{equation}
        v_p^q
        \equiv
        \langle \psi_p|\hat{V}|\psi_q\rangle
    \end{equation}
